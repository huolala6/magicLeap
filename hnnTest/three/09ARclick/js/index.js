...
if(version_str != "undefined" && version_str.length > 0) {
	...
	if(version > 5 && version < 9) {
		pgvSendClick({
			hottag: fileName + ".version.normal4"
		});
		window.location.href = locationJump
	}
}
//提供设备的加速信息，表示为定义在设备上的坐标系中的迪卡尔坐标。其还提供了设备在坐标系中的自转速率
window.addEventListener("devicemotion", function(event) {
	if(event.rotationRate.alpha || event.rotationRate.beta || event.rotationRate.gamma) {
		
	} else {
		pgvSendClick({
			hottag: fileName + ".version.normal5"
		});
		window.location.href = locationJump
	}
});
var pvpTime = stayTime({
	page: document,
	time: 1000,
	flag: fileName + ".staytime.loading",
	start: true
});
...
(function() {
	...
	var load = (function() {
		...
		function num(n) {
			...
			if(last_num < n) {
				...
				if(n == 100) {
					if(!Detector.webgl) {
						pgvSendClick({
							hottag: fileName + ".exposure.load_success"
						});
						pgvSendClick({
							hottag: fileName + ".version.normal1"
						});
						window.location.href = locationJump
					} else {
						pgvSendClick({
						...
						pgvSendClick({
							hottag: fileName + ".version.notnomal"
						});
		...
		var images = ["/pvp.jpg", "/DiaoChan_D_512.png", "/DiaoChan_Taikongcang_D_512.png", "/DiaoChan_Taikongcang_D_512.png", "/target.png", "/music_icon.png"];
	...
	function init() {
		...
		loader.load("//ossweb-img.qq.com/images/yxzj/act/a20161130ar/dc_mini.js", function(geometry, materials) {
			document.querySelector(".info").innerHTML = "�ƿ���գ�������";
			var config = {
				name: "diaochan",
				g: geometry,
				m: materials
	...
	function resetLocationAndRotation() {
		...
		if(need_3droom) {
			...
			var material = new THREE.MeshBasicMaterial({
				map: new THREE.TextureLoader().load("//ossweb-img.qq.com/images/yxzj/act/a20161130ar/pvp.jpg")
			});
		...
		var dc_tween5 = new TWEEN.Tween(poi_dc.position).to({
			...
			document.querySelector(".info").innerHTML = "ִ��֮�֣�һ���ӽ�������ҫ";
			diaochan.status = 2;
			dc_lock = true
		});
	...
	function animate() {
		...
		//点击3秒提示框
		if(meshPool["diaochan"].status == 3) {
			document.querySelector(".target").innerHTML = "�����׼����3�룬<br>���ɽ���";
			$(".target").show();
			if(Math.abs(dt) < 0.3 && Math.abs(dt2) < 0.3) {
				if(!targetTimeStart) {
					...
					document.querySelector(".target").innerHTML = "�Ѷ�׼��������<b>3</b>�룡";
					$(".target").addClass("flash");
					if(targetTime >= 1000 && targetTime < 2000) {
						document.querySelector(".target").innerHTML = "�Ѷ�׼��������<b>2</b>�룡"
					} else {
						if(targetTime >= 2000 && targetTime < 3000) {
							document.querySelector(".target").innerHTML = "�Ѷ�׼��������<b>1</b>�룡"
						} else {
							if(targetTime > 3000) {
								pgvSendClick({
									hottag: fileName + ".target.dc"
								});
								pvpTime.setFlag(fileName + ".staytime.after_jc");
								document.querySelector(".target").innerHTML = "�ɹ�����";
								setTimeout(function() {
									$(".target").hide();
	...
	function getMedia() {
		...
		if(navigator.getUserMedia) {
			...
			need_3droom = true;
			pgvSendClick({
				hottag: fileName + ".version.3droom2"
			})
		}
	}
	...
	function successFunc(stream) {
		...
		pgvSendClick({
			hottag: fileName + ".version.getusermedia"
		});
		video.play()
	}
	...
	function errorFunc(e) { 
		need_3droom = true;
		pgvSendClick({
			hottag: fileName + ".version.3droom1"
		});
		console.log("Error��" + e)
	}
	getMedia()
...
console.error = (function(origin) {
	return function(errorlog) {
		if(/THREE/.test(errorlog)) {
			pgvSendClick({
				hottag: fileName + ".version.normal2"
			});
			window.location.href = locationJump
		}
	}
...
console.warn = (function(origin) {
	return function(errorlog) {
		if(/^THREE.WebGLRenderer:$/.test(errorlog)) {
			pgvSendClick({
				hottag: fileName + ".version.normal3"
			});
			window.location.href = locationJump
		}
	}
...
document.ontouchstart = function(evt) {
	if(evt.target.tagName == "B" || evt.target.tagName == "EM") {
		if(music.paused) {
			...
			pgvSendClick({
				hottag: fileName + ".btn.music_on"
			})
		} else {
			music.pause();
			musicIcon.className = "music";
			pgvSendClick({
				hottag: fileName + ".btn.music_off"
			})
		}
	}
	evt.preventDefault()
